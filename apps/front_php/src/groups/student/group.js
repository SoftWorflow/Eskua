...
async function loadHomeContent() {
    ...
    await authenticatedFetch('/api/student/getGroupHomeData.php', {
        method: 'GET'
    }).then(res => res.json())
    .then(data => {
        if (!data.ok) {
            console.error('Error del servidor:', data.error);
            console.error(data.allowedRoles);
        ...
        history.replaceState(null, null, `/groups/student/?groupId=${group.id}`);
        groupNavbar.updateGroupNavbarHref();
